...
def test_find_target_files_filters_and_sorts(tmp_path):
    ...
    (tmp_path / "e.xlsx").write_bytes(b"")           # xlsx
    (tmp_path / "f.xls").write_bytes(b"")            # xls
    (tmp_path / "sub").mkdir()                       # ディレクトリは無視
    (tmp_path / "sub" / "g.docx").write_bytes(b"")  # ネストは無視
...
def test_find_target_files_ignores_dotfiles(tmp_path):
    (tmp_path / ".gitkeep").write_bytes(b"")  # ドットファイルは無視
    (tmp_path / "a.docx").write_bytes(b"")

    result = convert.find_target_files(tmp_path)

    assert result == [tmp_path / "a.docx"]
