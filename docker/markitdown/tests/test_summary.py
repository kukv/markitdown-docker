...
def test_format_summary_all_zero_shows_mount_hint():
    result = convert.ConversionResult(succeeded=[], failed=[], skipped=[])
...
def test_format_summary_with_skipped_omits_mount_hint():
    result = convert.ConversionResult(
        succeeded=[], failed=[], skipped=[Path("note.txt")]
    )

    text = convert.format_summary(result)

    assert "マウント" not in text
